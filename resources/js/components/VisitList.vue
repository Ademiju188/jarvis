<template>
    <div class="space-y-6">
        ...
        <transition enter-active-class="transition-all duration-500 ease-out"
            ...
            enter-to-class="opacity-100 transform translateY(0)" appear>
        ...
        <!-- Filters -->
        <transition enter-active-class="transition-all duration-500 ease-out"
            enter-from-class="opacity-0 transform translateY(20px)" enter-to-class="opacity-100 transform translateY(0)"
            appear>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 p-4 hover-lift">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Search</label>
                        <input type="text" v-model="search" placeholder="Search by school or consultant..."
        ...
        <!-- Visits List -->
        <transition enter-active-class="transition-all duration-600 ease-out"
            enter-from-class="opacity-0 transform translateY(30px)" enter-to-class="opacity-100 transform translateY(0)"
            appear>
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 hover-lift">
                ...
                <div v-else class="divide-y divide-gray-200">
                    <transition-group name="visit-list" tag="div" class="divide-y divide-gray-200">
                        <div v-for="(visit, index) in filteredVisits" :key="visit.id"
                            class="p-6 hover:bg-gray-50 transition-all duration-300 cursor-pointer hover-lift"
                            @click="$router.push(`/visits/${visit.id}`)" :style="{ animationDelay: `${index * 0.1}s` }">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    ...
                                    <div class="mt-2 flex items-center space-x-4 text-sm text-gray-500">
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <span class="text-xs text-gray-500">Headteacher: {{ visit.school.headteacher
                                                ? visit.school.headteacher.name : 'N/A' }}</span>
                                        </div>
                                        <div class="flex items-center">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                            ...
                                            <span class="text-xs text-gray-500">Created: {{
                                                formatDateTime(visit.created_at) }}</span>
                                        </div>
                                    </div>
                                ...
                                <div class="flex items-center space-x-2">
                                    <button @click.stop="downloadPdf(visit.id)"
                                    ...
                                    <button @click.stop="copyShareLink(visit.share_token)"
                                    ...
                                    <button @click.stop="openStatusModal(visit)"
                                        class="p-2 text-gray-400 hover:text-blue-600 transition-all duration-300 hover-lift cursor-pointer"
                                        title="Change Status">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4" @click.stop>
                <div class="p-6">
                    ...
                    <div class="mb-4">
                        ...
                        <span :class="getStatusClass(selectedVisit.status)"
                    ...
                    <div class="space-y-2">
                        ...
                        <button v-for="status in ['draft', 'pending_review', 'approved', 'finalised']" :key="status"
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <svg v-if="selectedVisit.status === status" class="w-5 h-5 text-blue-500" fill="none"
